...
4. 成熟しており、破壊的な変更が少ない。
5. かなりの回数並列処理を使っている割にはプログラマ側で意識しなくていいことが多い
...
xargsで新しいプロセスをガット作ってけっけを取得する

cat | xargs -n 1


2. 親プロセス、子プロセスを意識する必要がある
コマンド実行時に新しくプロセスを作って実行しているので、
引数で指定しなくてもそのプロセスの色んなところから参照したいならこういう環境変数を作って
みたいな感じ。

普通にWebApplication使ってるだけならあまり意識しなくて良いこと知らないとだめ。

3. 標準出力とエラーコードを意識して書く必要がある。

4. 共通関数だけまとめて他のファイルに押し出すのが難しい
bsdとgnuの差、共通関数から共通関数を呼ぶときのsource
.bash_profileから関数読み込みの処理をsourceで呼んでも関数が
引き継がれないので、その都度読みにいく必要がある。

5. 処理のたびにダブルクォート、バックスラッシュが外れる。
6. awkの闇が深い...
7. BSDとGNUでコマンドが違う
8. set -uをとりあえず使うというアンチパターン
9. trapという処理が独特
...
aliasで定義したものは対話形式のbashでしか、
aliasが聞かない
どういうことかというと、
bashには対話形式と非対話形式があって、

ちょっとここだけ対話で試したいとかで、
awkを触ったらgawkでシェルスクリプトに書かれているのは
aliasが気kないからawkになるみたいなわかりづらいバグを作る必要がある。

例えば、関数内やコマンド内で一時的に
関数を作ってそれをaliasの変わりにするみたいなやり方が良い。
それだったらスクリプトみてもここはgawkが実行されている、
...
nodejsからbashなどのシェルスクリプトを実行するためのframework.
シェルスクリプトのアンチでなくて、バッチ処理のダメなところを
nodejsでラップすることでバッチ処理のダメなところを補おうという考え方。

一番ダメだと思うところはnodejsでラッピングしてるのに、
bashの並列処理をnodejs側で書く必要があるので、手間が増えている。
...
外部にAPIを公開するときにクライアントライブラリとして最初に選ばれる言語は
多いのはpython。
...
割と外部にAPIを公開するときにそういうツールに選ばれるのは
少ない。
昔からあるGMOのあおぞら銀行とかはjava。
まあ叩く部分だけjavaで作ってコマンドとして外にだすというのもあり。
...
とりあえず動かすには向かないが、言語として混乱する仕様が少ない

ビジネスロジックだけ
書いててダルいは事実。しかし、愚直に書くべき。
マルチモジュールとかレポジトリ周りの方針,ツール周りが心配。
googleさんも社内で使ってるらしいので、急に捨てられるはまずない。

エンジニアの単価が高いのと
シニアレベルのエンジニアがちゃんときっちりしてるか？
でうまくいくか決まりそう。




レポジトリ、モジュールの考え方が独特でおそらく一度はみな混乱する。
